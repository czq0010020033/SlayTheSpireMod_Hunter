...
public class DefectPetCall
  extends PetCall
{
  public static final String ID = "DefectPetCall";
 // private static final CardStrings cardStrings = CardCrawlGame.languagePack.getCardStrings("Defend_R");
  ...
  public DefectPetCall()
  {
    ...
    super(ID, NAME, DESCRIPTION, 2, CardColor.COLORLESS, CardRarity.SPECIAL);
   // this.baseBlock = 5;
    this.exhaust = false;
	...
	public void use(AbstractPlayer p, AbstractMonster m) {
		 AbstractMonster pet = 	CallPet();
			if(pet != null && (pet instanceof DefectPet) && this.upgraded )
			((Pet)pet).passiveAmount += 2;
	}
 ...
 /* public void beforeSpawn(AbstractMonster pet){
  ...
  {
    return makePetCopy(new DefectPetCall());
  }
  public void upgrade() {
		if (!this.upgraded) {
			upgradeName();
